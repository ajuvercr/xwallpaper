...
static size_t add_buffer(wp_buffer_t **bufs, size_t *count, wp_buffer_t buf) {
  size_t i;

  for (i = 0; i < *count; i++)
    if ((*bufs)[i].st_dev == buf.st_dev && (*bufs)[i].st_ino == buf.st_ino)
      break;
...
static void add_option(wp_config_t *config, wp_option_t option) {
  size_t i;
  wp_option_t *o;
...
static void init_buffers(wp_config_t *config) {
  ...
  for (i = 0; i < config->count; i++) {
    ...
    if ((buffer.fp = fopen(config->options[i].filename, "rb")) == NULL)
...
static int parse_screen(char *screen) {
  char *endptr;
  long value;
...
static int parse_box(char *s, wp_box_t **box) {
  wp_box_t b;
  char *endptr;
  char sign;
  uint16_t val;
  ...
  switch (
      sscanf(s, "%hux%hu+%hu+%hu", &b.width, &b.height, &b.x_off, &b.y_off)) {
  case 2:
    b.x_off = 0;
    b.y_off = 0;
    break;
  ...
  if (UINT16_MAX - b.width < b.x_off || UINT16_MAX - b.height < b.y_off ||
...
wp_config_t *parse_config(char **argv) {
  wp_config_t *config;
  wp_option_t last;
  ...
  while (*argv != NULL) {
    if (strcmp(argv[0], "--daemon") == 0) {
      ...
      if (*++argv == NULL) {
        warnx("missing argument for time");
        return NULL;
      }
      config->time = atoi(*argv);
