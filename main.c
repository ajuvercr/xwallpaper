...
static uint32_t get_max_rows_per_request(xcb_connection_t *c,
  ...
  row_len = (image->stride + image->scanline_pad - 1) & -image->scanline_pad;
...
static void load_pixman_images(xcb_connection_t *c, xcb_screen_t *screen,
  ...
  for (opt = options; opt != NULL && opt->filename != NULL; opt++)
    if (opt->buffer->pixman_image == NULL) {
      ...
      height = pixman_image_get_height(current_image(img));
      width = pixman_image_get_width(current_image(img));
      ...
      if (opt->trim != NULL) {
        ...
        if (height < trim->y_off + trim->height ||
          ...
          errx(1, "%s is smaller than trim box", opt->filename);
      }
    }
}
  ...
  for (off_y = 0; off_y < output->height; off_y += src_height) {
    ...
    for (off_x = 0; off_x < output->width; off_x += src_width) {
      ...
      debug("tiling %s for %s (area %dx%d+%d+%d)\n", option->filename,
            output->name != NULL ? output->name : "screen", w, h, off_x, off_y);
      pixman_image_composite(PIXMAN_OP_CONJOINT_SRC, pixman_image, NULL, dest,
                             src_x, src_y, 0, 0, off_x, off_y, w, h);
    }
  }
}
...
static void transform(pixman_image_t *dest, wp_output_t *output,
  ...
  if (mode == MODE_FOCUS) {
    ...
    debug("focus on trim box %hux%hu%+.0f%+.0f of %hux%hu for "
          "output %hux%hu\n",
          src_width, src_height, off_x, off_y, pix_width, pix_height, xcb_width,
          xcb_height);
  ...
  pixman_f_transform_init_translate(&ftransform, translate_x, translate_y);
  ...
  debug("composing %s for %s (area %dx%d+%d+%d) (mode %d)\n", option->filename,
        ...
        output->height, 0, 0, option->mode);
  pixman_image_composite(PIXMAN_OP_CONJOINT_SRC, pixman_image, NULL, dest, 0, 0,
                         0, 0, 0, 0, output->width, output->height);
}
...
static void process_atoms(xcb_connection_t *c, xcb_screen_t *screen,
  ...
  atom_cookie[0] =
      xcb_intern_atom(c, 0, sizeof(ATOM_ESETROOT) - 1, ATOM_ESETROOT);
  atom_cookie[1] =
      xcb_intern_atom(c, 0, sizeof(ATOM_XSETROOT) - 1, ATOM_XSETROOT);
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (property_reply[i] != NULL && property_reply[i]->type == XCB_ATOM_PIXMAP)
...
static void usage(void) {
  fprintf(
      ...
      "  [--maximize <file>] [--stretch <file>] [--tile <file>] [--zoom "
      "<file>] [--time <time>]\n"
      "  [--version]\n");
  exit(1);
}
